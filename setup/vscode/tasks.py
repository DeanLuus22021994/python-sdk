...
class VSCodeTasksManager:
    ...
    def should_create_tasks(self) -> bool:
        ...
        try:
            ...
            has_test_task = any("test" in task.get("label", "").lower() for task in tasks)
            has_build_task = any(
                task.get("group") == "build" or
                (isinstance(task.get("group"), dict) and task.get("group", {}).get("kind") == "build")
                for task in tasks
            )
    ...
    def update_tasks(self, updates: dict[str, Any], merge: bool = True) -> bool:
        ...
        try:
            ...
            # Write updated configuration
            with open(# filepath: c:\Projects\python-sdk\setup\vscode\tasks.py
"""
VS Code tasks configuration manager.
